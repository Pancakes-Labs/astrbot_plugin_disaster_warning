...
class WeatherAggregationService:
    ...
    def should_aggregate(
        ...
        flush_on_red = bool(agg_config.get("flush_on_red", True))
    ...
    async def _flush_via_forward(
        ...
        先尝试合并转发，成功则不限流；
        平台不支持合并转发时由框架层返回失败，
        插件捕获异常后降级为逐条文本发送，此时才启用限流机制。
        """
        rate_limit_enabled = bool(agg_config.get("rate_limit_enabled", True))
        max_messages = int(agg_config.get("rate_limit_max_messages", 3))
        window_seconds = float(agg_config.get("rate_limit_window_seconds", 900))
        ...
        # 分批构建合并转发节点
        batches = [
            entries[i : i + _MAX_NODES_PER_FORWARD]
            for i in range(0, len(entries), _MAX_NODES_PER_FORWARD)
        ]

        for batch_idx, batch in enumerate(batches):
            try:
                # 先尝试合并转发，成功则不限流
                await self._flush_callback(session, batch, self._config, mode="forward")
            except Exception as e:
                logger.debug(
                    f"[灾害预警] 合并转发推送失败 ({session}), 降级为逐条发送: {e}"
                )
                # 合并转发失败，降级为逐条发送
                # 此时才启用限流：不支持合并转发的平台需要限制推送数量
                if rate_limit_enabled:
                    now = time.time()
                    counter = self._rate_limit_counters.setdefault(session, [])
                    # 清理过期记录
                    counter[:] = [ts for ts in counter if now - ts < window_seconds]
                    available_slots = max(0, max_messages - len(counter))

                    if available_slots == 0:
                        plugin_logger.info(
                            f"[灾害预警] 气象预警聚合推送降级限流: {len(batch)} 条, "
                            f"限流窗口已满 ({max_messages}/{int(window_seconds)}s), "
                            f"全部丢弃 ({session})",
                            is_event_linked=True,
                            event_stream="weather_alarm",
                        )
                    ...
                    # 限流：只取前 available_slots 条
                    if len(batch) > available_slots:
                        dropped = batch[available_slots:]
                        batch = batch[:available_slots]
                        dropped_summary = ", ".join(
                            f"{e.title}({e.color_name})" for e in dropped[:5]
                        )
                        plugin_logger.info(
                            f"[灾害预警] 气象预警聚合推送降级限流: "
                            f"限流推送 {len(batch)} 条, 丢弃 {len(dropped)} 条 "
                            f"({session}), 丢弃: {dropped_summary}",
                            is_event_linked=True,
                            event_stream="weather_alarm",
                        )
                ...
                for entry in batch:
                    try:
                        await self._flush_callback(
                            session, [entry], self._config, mode="single"
                        )
                        if rate_limit_enabled:
                            now = time.time()
                            counter = self._rate_limit_counters.setdefault(session, [])
                        ...
                        logger.error(f"[灾害预警] 逐条降级发送也失败: {single_err}")
