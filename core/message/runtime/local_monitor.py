...
class LocalMonitor:
    ...
    def _resolve_threshold(self) -> tuple[float, str]:
        ...
        if self._get_system() == SYSTEM_JMA:
            return float(self.shindo_threshold), "震度"
        return float(self.intensity_threshold), "烈度"

    def _get_system(self) -> str | None:
        """解析本地坐标应使用的强度体系（带坐标变更缓存）。"""
        try:
            return resolve_intensity_system(
                float(self.latitude),
                float(self.longitude),
                self.intensity_system,
            )
        except Exception as exc:
            logger.debug(f"[灾害预警] 本地强度体系解析失败: {exc}")
            return SYSTEM_CENC
    ...
    def _estimate_intensity(
        ...
        中国体系（cenc）：使用烈度衰减式，按本地坐标经度自动切换东西部参数；
        ...
        # CENC 烈度：event_longitude 传本地经度，自动切换东西部衰减参数
        return IntensityCalculator.calculate_estimated_intensity(
            magnitude,
            distance,
            depth,
            event_longitude=self.longitude,
        )
